...
instance Foldable BTree where
    foldr f b Nil = b
    foldr f b (Node x left right) = foldr f b' left
        where
            b' = foldr f b right
            b'' = f x b'
...
getLevel (Node _ left right) n = getLevel left (n - 1) ++ getLevel right (n - 1)

traverseBFS :: BTree a -> [a]
traverseBFS tree = concat $ takeWhile (not . null) [getLevel tree level | level <- [0..]]
